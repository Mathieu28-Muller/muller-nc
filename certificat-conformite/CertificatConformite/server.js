...
function getAlternatives(section) {
  ...
  if (type === 'classeL') {
    ...
    if (!hasCelero && hasSono) return ['0225SONO','0225LAN5','0223LAN5','0226LAN5','0227LAN5'];
    if (hasCelero && hasSono)  return ['0225SONOCELERO','0223CELERO','0226CELERO','0227CELERO'];
...
app.post('/api/demandes/:id/valider', async (req, res) => {
  ...
  if (!sections.length) return res.status(400).json({ error: 'Aucune section' });

  try {
    const attestations = [];

    for (let i = 0; i < sections.length; i++) {
      // Chaque section = un numéro distinct + un PDF distinct
      const numero = genNumero();
      const pdfPath = path.join(ARCHIVES_DIR, `${numero}.pdf`);
      await generateOnePDF(demande, sections[i], numero, pdfPath);
      attestations.push({ numero, pdfUrl: `/api/archives/${numero}.pdf`, sectionIndex: i });
    }
    ...
    if (idx2 >= 0) {
      ...
      db2.demandes[idx2].attestations = attestations;
      db2.demandes[idx2].numero     = attestations[0].numero; // compat
      db2.demandes[idx2].valideeAt  = new Date().toISOString();
      db2.demandes[idx2].envoyeeAt  = new Date().toISOString(); // date/heure d'envoi (= validation tant que l'email auto n'est pas activé)
      writeDB(db2);
    }

    res.json({ ok: true, attestations, numero: attestations[0].numero });
  } catch (e) {
