...
async function fetchResults(q) {
  const res = await fetch(`/certificat-conformite/api/search?q=${encodeURIComponent(q)}`);
...
async function selectClient(r) {
  ...
  if (id) {
    const res = await fetch(`/certificat-conformite/api/client/${encodeURIComponent(id)}`);
...
async function submitDemande() {
  ...
  if (blockChecked('block-cl')) {
    ...
    if (hasCelero && hasSono) {
      ...
      const sonoCert = getValue('cl-son-cert');
      const sonoCertSuffix = (sonoCert.match(/(\d{4})$/) || ['',''])[1];
      // Feuille combinée uniquement si le cert sonomètre est un cert combiné (0223/0225/0226/0227)
      // Sinon (ex: 0212, 0190…) → deux attestations distinctes
      const isCombinedCert = ['0223','0225','0226','0227'].includes(sonoCertSuffix);
      ...
      if (isCombinedCert) {
        ...
        // Certs individuels → 2 sections séparées = 2 attestations
  ...
  try {
    const res = await fetch('/certificat-conformite/api/demandes', {
      method: 'POST',
...
async function confirmReplacement() {
  ...
  try {
    const res = await fetch('/certificat-conformite/api/equipement/replace-sn', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
